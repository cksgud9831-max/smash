...
simulation\run_smash_fcs.bat
  ...
  2 워크스페이스 링크 연결 (link)
  3 colcon 빌드 (build)
  4 [비사격] 시뮬레이션 + 스코프 뷰어 (탐지·조준만, 격발 없음)
  5 [수동 격발] 시뮬레이션 + 스코프 뷰어 (조준 수동, READY 에서 Space 격발)
  6 [자동 격발] 시뮬레이션 + 스코프 뷰어 (조준 수동, READY 되면 격발만 자동)
  7 3단계 격발 판정 오프라인 검증 (ROS 2 및 Gazebo 불필요)
  8 스코프 뷰어 창만 다시 열기 (4~6번 실행 중 창을 닫았을 때)
  9 Gazebo 3D GUI 창 토글 (헤드리스/GUI 모드 전환)
```

- 포탑은 표적을 자동으로 따라가지 않는다. 뷰어에서 방향키/WASD(Shift 는 큰 스텝)로 조준선을 리드 레티클에 맞춰야 READY 가 되고, 5·6번 모두 READY 일 때만 격발된다. T 는 대공 경계 자세, R 은 수평 리셋.
- 포탑은 대공 경계 자세(tilt 0.38 rad)로 기동해 기본 표적이 처음부터 화면에 들어온다 (`smash_fcs.yaml` 의 `search_tilt_rad`).
- 8번은 시뮬레이터를 켜지 않는다. 뷰어 창 대신 브라우저로 `http://localhost:9999` 에 접속해도 된다.
