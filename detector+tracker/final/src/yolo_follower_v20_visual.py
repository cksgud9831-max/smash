...
def evaluate_tracker(params, video_path=VIDEO_PATH, model_path=MODEL_PATH, gt_json_path=None, model=None, max_frames=None):
    ...
    try:
        ...
        # Visualization only - tracking logic is unchanged
        vis_path = "output_video/v20_FINAL_demo.mp4"
        os.makedirs("output_video", exist_ok=True)

        vis_fps = cap.get(cv2.CAP_PROP_FPS)
        if vis_fps <= 0:
            vis_fps = REALTIME_INPUT_FPS
        ...
        while True:
            ...
            vis = search_frame.copy()

            cv2.putText(
                vis,
                f"SEARCH | YOLO every {INITIAL_SEARCH_INTERVAL}f | "
                f"confirm {search_confirm_hits}/{SEARCH_CONFIRM_HITS}",
                (30, 50),
        ...
        cv2.putText(
            vis,
            f"Frame {initial_acquisition_frame}",
            (30, 90),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (255, 255, 255),
        ...
        while True:
            ...
            # 이전 TRACK frame에 대한 최종 bbox 기록
            if prev_vis_frame is not None:
                vis = prev_vis_frame.copy()

                vx, vy, vw, vh = map(int, smooth_bbox)
                ...
                cv2.putText(
                    vis,
                    "TRACK | CPU Optical Flow + Async TensorRT YOLO",
                    (30, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.85,
                    (0, 255, 0),
                ...
                cv2.putText(
                    vis,
                    f"Frame {prev_vis_index} | YOLO correction 20f",
                    (30, 90),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.75,
                    (255, 255, 255),
            ...
            # 현재 프레임은 tracking 처리가 끝난 다음 iteration에서 기록
            prev_vis_frame = raw_frame.copy()
            prev_vis_index = frame_index
        ...
        # 마지막 TRACK frame 기록
        if 'vis_writer' in locals() and vis_writer is not None:

            if (
                'prev_vis_frame' in locals()
                and prev_vis_frame is not None
                and 'smooth_bbox' in locals()
            ):
                vis = prev_vis_frame.copy()
