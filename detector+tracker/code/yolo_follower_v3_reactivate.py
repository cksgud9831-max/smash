...
# ====================== 기본 설정 ====================== #
VIDEO_PATH = r"C:\Users\jk091\anti-uav-yolo11s\datasets\Anti-UAV300\test\20190925_111757_1_1\visible.mp4"
MODEL_PATH = r"C:\Users\jk091\anti-uav-yolo11s\runs\detect\runs\yolo11s_ga_final-3\weights\best.pt"
...
PERIODIC_DETECT_INTERVAL = 30  # GA-efficient candidate: periodic YOLO correction interval
...
GA_PARAM_BOUNDS = {
    ...
    "PERIODIC_DETECT_INTERVAL": (30, 120),
...
def evaluate_tracker(params, video_path=VIDEO_PATH, model_path=MODEL_PATH, gt_json_path=None, model=None, max_frames=None):
    ...
    try:
        ...
        eval_model = model if model is not None else YOLO(model_path)
        start_time = time.time()

        bbox, score, mode = yolo_detect(first_frame, eval_model, roi_bbox=None)
        if bbox is None:
            return {
                "fitness": -1.0,
        ...
        while True:
            ...
            elif frame_index % PERIODIC_DETECT_INTERVAL == 0:
                need_redetect = True
                redetect_reason = "periodic"

            if need_redetect:
                if redetect_reason != "periodic":
                    # GA metric: count non-periodic recovery/re-acquisition events.
                    recovery_count += 1
        ...
        metrics = {
            ...
            "fps": fps,
            "average_fps": fps,
            "fps_normalized": min(fps / 30.0, 1.0),
